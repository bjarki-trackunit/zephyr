...
int modem_socket_init(struct modem_socket_config *cfg, struct modem_socket *sockets,
			      size_t sockets_len, size_t base_socket_id, bool assign_id,
			      const struct socket_op_vtable *vtable);
 ...
 *
 * @return true if the socket has been allocated, otherwise false
 */
 ...
 *
 * @details If the assign_id modem socket configuration is not set, this function is used to
 * determine if the socket id has been assigned. Otherwise, a call to modem_socket_is_allocated
 * will suffice as the id is automatically assigned during allocation.
 *
 * @param cfg The modem socket config which the modem socket belongs to
